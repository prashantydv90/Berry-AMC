...
export const ClientDetails = () => {
    ...
    const handleDeleteInvestment = async (type, id) => {
        ...
        try {
            ...
            const url = `https://berry-amc.onrender.com/api/deleteinvestment/${type}/${id}`;
            await axios.delete(url, { withCredentials: true });

            // Refresh client data
            const res = await axios.get(
                `https://berry-amc.onrender.com/api/get1ClientDetails/${client._id}`,
    ...
    const handleDeleteInterest = async (id) => {
        ...
        try {
            ...
            await axios.delete(url, { withCredentials: true });
            // Refresh client data
            const res = await axios.get(`https://berry-amc.onrender.com/api/get1ClientDetails/${client._id}`, { withCredentials: true });
            setClient(res.data.data);
        } catch (err) {
            console.error("Error deleting investment:", err);
    ...
    const handleResetMFInvestment = async (id) => {
        ...
        try {
            const url = `https://berry-amc.onrender.com/api/resetInvestment/${investmentType}/${id}`;
            await axios.delete(url, { withCredentials: true });
            const res = await axios.get(`https://berry-amc.onrender.com/api/get1ClientDetails/${client._id}`, { withCredentials: true });
                                ...
                                <div className="md:min-w-[600px] min-w-[800px]">
                                    ...
                                    {client?.FDInvestments?.map((p) => (
                                        <div key={p._id} className="grid grid-cols-6 items-center py-3 px-3 bg-white border-b border-zinc-200 text-sm">
                                            <div>{formatDate(p?.date)}</div>
                                            <div className="ml-2">₹{toIndianFormat(p?.investedValue)}</div>
