...
export const startIpoStatusCron = () => {
  ...
  cron.schedule("0 0 * * *", async () => {
    ...
    console.log(`[CRON] IPO status update started at ${today.toISOString()}`);

    try {
      const result = await IPO.updateMany({}, [
        {
          $set: {
            status: {
              $cond: [
                { $gt: ["$issueStartDate", today] },
                "Upcoming",
                {
                  $cond: [
                    {
                      $and: [
                        { $lte: ["$issueStartDate", today] },
                        { $gte: ["$issueEndDate", today] }
                      ]
                    },
                    "Open",
                    "Closed"
      ...
      console.log(
        `[CRON] IPO status update completed | Modified: ${result.modifiedCount}`
      );
    } catch (error) {
      console.error("[CRON] IPO status update failed:", error.message);
    }
  });
};