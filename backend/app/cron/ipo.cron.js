...
const fetchIPOData = async (retries = 3) => {
  try {
    ...
    console.warn(
      `[CRON] Fetch failed. Retrying... (${retries} left)`
    );

    await sleep(3000); // 3 sec backoff
    return fetchIPOData(retries - 1);
  }
};
...
export const startIpoCron = () => {
  cron.schedule("*/1 * * * *", async () => {
    ...
    try {
      const ipoList = await fetchIPOData();
      ...
      console.log(
        `[CRON] IPO fetch completed | Records: ${processedCount} | Time: ${duration}ms`
      );
    } catch (error) {
      if (error.response) {
        console.error(
          `[CRON] HTTP Error: ${error.response.status}`
        );
      } else if (error.code === "ECONNABORTED") {
        console.error("[CRON] Request timed out");
      } else {
        console.error(
